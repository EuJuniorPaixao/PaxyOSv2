...
name: instalar
description: >
  Instala o PaxyOS no negócio do usuário. Entrevista sobre empresa, tom de voz,
  ...
  pedir explicitamente "rodar /instalar", "instalar o PaxyOS", "primeiro setup".
---

# /instalar — Instalação inicial do PaxyOS
...
Conferir o nome da pasta atual (`basename "$(pwd)"`). Se for `paxyos`, `PaxyOS`, `PaxyOS-main`, `paxyos-main` ou variação genérica:

> "Notei que a pasta atual ainda tem nome genérico ('<nome-atual>'). O ideal é a pasta ter o nome do seu negócio, não 'PaxyOS'. Quando terminarmos o setup, te lembro de renomear (é rápido — fechar VS Code, renomear a pasta no Finder/Explorer, abrir de novo). Bora seguir?"
