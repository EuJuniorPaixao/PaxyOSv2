...
name: salvar
description: >
  Salva o trabalho do PaxyOS no GitHub (commit + push). Na primeira vez configura o repositório
...
1. Perguntar:
   ...
   > 2. Não, vou criar agora — me dá um nome pro repositório (ex: meu-paxyos)"

2. **Se opção 1:** rodar `git init`, `git add .`, `git commit -m "Setup inicial do PaxyOS"`, `git branch -M main`, `git remote add origin <URL>`, `git push -u origin main`.
